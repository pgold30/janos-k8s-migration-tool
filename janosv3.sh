...
#read kubeversion
SEARCHPATH=/home/pablo/git/janos/test
kubeversion=1.16.0
...
function add_spec-selector () {
  ...
  echo $KIND
#  if [[ "$KIND" =~ "^(Deployment|Daemonset|Statefulset|ReplicaSet)$" ]]; then
  if [[ "$KIND" == *"Deployment"* ]]; then
	  echo "a"
    export METADATANAME=$(yq eval '.metadata.name' $FILE)
    yq -i eval '.spec.selector.matchLabels.app |= env(METADATANAME)' $FILE
  fi
  ##yq -i eval 'select(.kind == "ClusterRoleBinding") | .spec.selector.matchLabels.app |= "asasasas"' argocd-manager-role.yaml
